...
<p align="center">
  <img src="https://img.shields.io/badge/version-v0.10.0-blue.svg" alt="Version: v0.10.0">
...
[Claude Code](https://docs.claude.com/en/docs/claude-code/overview); the 77
...
- **Python 3.10+** — the 77 tools are stdlib Python.
...
- **77 stdlib modules** (`tools/checks/`, stdlib-only, JSON) — recon, active testing (injection, XSS incl. client-side DOM, SSRF, access control + 401/403 bypass, request smuggling, file upload, SOAP/XXE, rate limiting, JWT, AI/LLM surface, …), visual triage, authenticated testing, edge-egress rotation, and reporting. Full catalog: [`tools/checks/README.md`](tools/checks/README.md).
...
- **Chain exploitation** — the `exploiter` combines confirmed issues into full attack chains (JWT-forge→account takeover, SSRF→internal metadata, IDOR at scale). When a lead needs bespoke exploit code no fixed probe covers, it opens a gated, captured **exploit-dev lane** (one-off PoC under a gitignored scratch dir) — a LEAD until the verifier independently reproduces it. Both are off unless `mutation_testing: approved` + an authorization basis.
...
tools/checks/                                     77 stdlib modules (stdlib-only, JSON)
...
- **[tools/checks/README.md](tools/checks/README.md)** — the 77-tool catalog.
