...
`validation_status`. Then generate the deliverable:
`python tools/report-render/render_report.py engagements/<name>/findings.json --all`
→ `report.md` + `report.html` (dark). The light/print theme (for PDF) is OPT-IN: run a second
pass with `--theme light` → `report-light.html`. Never hand-edit the generated files.
...
2. Scope & authorization — `engagement` + `coverage`.
3. Methodology — `method`.
4. Findings — `findings[]` (ordered by severity Critical→Low).
5. Informational/hardening — `informational[]`.
6. Leads (unconfirmed) — `leads[]` (kept separate from findings).
7. Appendix — `evidence_index`.
...
- **`findings.json` is the SINGLE SOURCE OF TRUTH. Write it; generate everything
  ...
  1. Write `engagements/<name>/findings.json` with the Write tool — schema in
     ...
     `risk_tier`). Each CVSS/CWE/count is written AFTER you read its evidence
