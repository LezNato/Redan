...
| Reflected param, context + encoding check | XSS (reflection-grade, CWE-79) | `xss_scan.py` — raw reflection + landing context + **non-executing-sink detection** (`<textarea>`/`<title>`/comment → not executable) + encoding-neutralization = LEAD; DOM-XSS (source→sink) via `dom_probe.py --xss` |
| SPA (React/Angular/Vue) value flows hash/query → a DOM write | DOM-based XSS (WSTG-CLNT-01, CWE-79) | `dom_probe.py --xss` — INSTRUMENTS the DOM (hooks innerHTML/eval/document.write/setTimeout before page scripts), drives `location.hash`/query sources; a marked value reaching a sink = taint LEAD, a fired `alert()` = execution observed (verifier confirms attacker delivery) |
| `window.addEventListener('message', …)` handler present | Web Messaging / postMessage abuse (WSTG-CLNT-11) | `dom_probe.py --postmessage` — enumerates message handlers, flags one routing `event.data` into a sink with NO `event.origin` check (LEAD; verifier confirms a real cross-origin source) |
| Client-side deep-merge of URL/query into an object (SPA config/router) | Client-side prototype pollution (CWE-1321) | `dom_probe.py --protopollute` — `?__proto__[x]=` / `constructor[prototype]` carriers → `Object.prototype` pollution vs a clean control (SOURCE deterministic; gadget→DOM-XSS a further lead) |
...
| A path/endpoint returns 401/403 to a plain request | Access-control bypass (WSTG-ATHZ-01, CWE-284/285) | `forbidden_bypass.py` — path-normalization (`/admin/`, `//admin`, `/%2e/`, `..;/`, case) + URL-rewrite headers (`X-Original-URL`) + client-IP-spoof (`X-Forwarded-For: 127.0.0.1`) + verb swaps, each vs the original deny + a nonexistent-path control; a differing 2xx = LEAD (confirm it returns the protected content, not a shell) |
...
| AI chatbot / "ask"/"assistant" feature / `/v1/chat/completions`-style or `/api/chat` endpoint / an MCP server | Prompt injection / excessive agency / system-prompt leak / unauth AI exposure (OWASP LLM01/LLM06) | `llm_probe.py` — agnostic discovery (polyglot body) + a COMPUTED detection marker (`13*13`, un-forgeable by a reflector), then a computed-token battery: prompt-injection (override → `REDAN`+`17*17`, several framings) + Base64/reversed **filter-bypass** + **multi-turn/Crescendo** (gradual escalation that bypasses a latest-turn-only guardrail) + **indirect/data-channel** (the override hidden in a `retrieved data` field with a benign prompt → fires = the model trusted data as instructions, the RAG/agent confusion) + **tool-abuse** (`--oob`: the LLM is told to fetch a collaborator URL → a callback = SSRF-via-the-app's-LLM / excessive agency) + system-prompt-leak + unauth MCP `tools/list` + MCP **tool-poisoning**. Each a LEAD (`tool_abuse` the strongest — a real outbound request; still needs internal reach for confirmed SSRF) — instruction-following ≠ a finding until impact is shown; an unauthenticated LLM endpoint is INFORMATIONAL, not a lead. Remaining ceiling: a TRUE cross-request STORED injection (needs a known persisted-ingestion vector) |
