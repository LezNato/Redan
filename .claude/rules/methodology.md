...
| AI chatbot / "ask"/"assistant" feature / `/v1/chat/completions`-style or `/api/chat` endpoint / an MCP server | Prompt injection / system-prompt leak / unauth AI exposure (LLM01) | `llm_probe.py` — agnostic discovery (polyglot body) + a COMPUTED detection marker (`13*13`, un-forgeable by a reflector), then prompt-injection (override → `REDAN`+`17*17`) + system-prompt-leak + unauth MCP `tools/list`. Each a LEAD — instruction-following ≠ a finding until impact (downstream trust / tool access / system-prompt secrets) is shown; an unauthenticated LLM endpoint is INFORMATIONAL, not a lead |
