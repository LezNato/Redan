...
| AI chatbot / "ask"/"assistant" feature / `/v1/chat/completions`-style or `/api/chat` endpoint / an MCP server | Prompt injection / excessive agency / system-prompt leak / unauth AI exposure (OWASP LLM01/LLM06) | `llm_probe.py` — agnostic discovery (polyglot body) + a COMPUTED detection marker (`13*13`, un-forgeable by a reflector), then a computed-token battery: prompt-injection (override → `REDAN`+`17*17`, several framings) + Base64/reversed **filter-bypass** + **tool-abuse** (`--oob`: the LLM is told to fetch a collaborator URL → a callback = SSRF-via-the-app's-LLM / excessive agency) + system-prompt-leak + unauth MCP `tools/list` + MCP **tool-poisoning**. Each a LEAD (`tool_abuse` the strongest — a real outbound request; still needs internal reach for confirmed SSRF) — instruction-following ≠ a finding until impact is shown; an unauthenticated LLM endpoint is INFORMATIONAL, not a lead. Not covered: multi-turn/Crescendo + stored injection |
