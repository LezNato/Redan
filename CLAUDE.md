...
A comprehensive web-only black-box pentest ensemble: **78 stdlib modules**, **8 agents**, a
...
### Tooling (`tools/checks/` (78 stdlib modules) + `tools/report-render/`)
...
`flow_map` (business-process + expected-authz **oracle skeleton** — flows/anon-access-matrix/candidate-invariants → annotated into `business_process_map.json`; the intent model black-box logic/authz testing tests against, filling the unauth gap `roles.json` covers authed).
